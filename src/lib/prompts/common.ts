...
export function getCommonRulesForUserPrompt(
  ...
  const mustEmojiOrBang = lowScore
    ? "* 絵文字（😊など）や「！」は使わないこと\n"
    : "* 【必須】絵文字（😊など）か「！」を必ず1つ以上含めること。改行は0〜2箇所。改行なしでも自然なら入れなくてよい（実際の口コミの36%は改行なし）\n";
...
■ 文章ルール
* 80〜150文字程度を基本とする。長くても200文字まで。短くて自然なら80文字未満でもよい（実際の口コミの平均は127文字、半数近くが100文字未満）
* 回答に含まれる語句（地名・固有名詞など）は削除しないこと
* 感情表現は1回まで
${lowScore ? "* 絵文字（😊など）や「！」は使わないこと\n" : "* 絵文字（😊など）か「！」を少なくとも1つは入れること（自然な範囲で）\n"}
* 文章は患者様本人の視点で書くこと
* 「この店は〜」「こちらの〜は」と店を主語にしないこと
${lowScore ? "* 改行は0〜2箇所。改行なしでも自然なら入れなくてよい\n" : "* 改行は0〜2箇所。改行なしでも自然なら入れなくてよい（実際の口コミの36%は改行なし）\n"}
...
export function getCommonOutputFormat(satisfaction: Satisfaction = null): string {
  if (isLowScoreSatisfaction(satisfaction)) {
    return "■ 出力形式\n口コミ本文のみを出力すること。説明・補足・前置きは一切不要。\n※絵文字（😊など）や「！」は使わない。改行は0〜2箇所。改行なしでも自然なら入れなくてよい。";
  }
  return "■ 出力形式\n口コミ本文のみを出力すること。説明・補足・前置きは一切不要。\n※必ず絵文字（😊など）か「！」を1つ以上入れること。改行は0〜2箇所。改行なしでも自然なら入れなくてよい（実際の口コミの36%は改行なし）。";
}
